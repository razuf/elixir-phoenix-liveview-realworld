defmodule RealWorldWeb.MainLiveTest do
  ...
  describe "click Sign in" do
    ...
    test "click Sign in from home", %{conn: conn} do
      ...
      assert html =~
               ~s|<li class=\"nav-item \"><a class=\"nav-link\" data-phx-link=\"patch\" data-phx-link-state=\"push\" href=\"/register\"> Sign up </a></li>|

      assert live
             |> element("a", "Sign up")
             |> render_click()
             |> clean_html() =~
               ~s|<li class=\"nav-item active\"><a class=\"nav-link\" data-phx-link=\"patch\" data-phx-link-state=\"push\" href=\"/register\"> Sign up </a></li>|
    end
  end
